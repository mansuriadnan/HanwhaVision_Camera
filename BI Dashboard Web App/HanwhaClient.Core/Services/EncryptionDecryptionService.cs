...
namespace HanwhaClient.Core.Services
{
    ...
    {
        private string EncryptionDecryptionService_Key;
        public EncryptionDecryptionService(IConfiguration configuration)
        {
            //this.config = config.Value;
            EncryptionDecryptionService_Key = configuration.GetSection("EncryptionDecryptionService:Key").Value;
        }
...
        public EncryptionDecryptionService()
        ...
        public string EncryptString(string plainText)
        {
            ...
            {
                aes.Key = Encoding.UTF8.GetBytes("1ED44028BD9A4BF68C9121D73C6060FA");
                aes.IV = iv;
        ...
        public string DecryptString(string cipherText)
        {
            ...
            using (Aes aes = Aes.Create())
            {
                aes.Key = Encoding.UTF8.GetBytes(EncryptionDecryptionService_Key);
                aes.IV = iv;
