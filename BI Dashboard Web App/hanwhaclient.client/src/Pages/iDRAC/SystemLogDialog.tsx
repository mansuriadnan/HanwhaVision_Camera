...
import { fetchIdracServerEventData, fetchIdracSytemData } from '../../services/iDRACService';
import { IEventLogDetail, IIdracServerEventPayload, IIdracServerSystemPayload, ISystemLogDetail } from '../../interfaces/IManageiDRAC';
...
const SystemLogDialog: React.FC<Props> = ({ open, onClose, serverID }) => {
    const [paginationModel, setPaginationModel] = useState<GridPaginationModel>({
       pageSize: 10,
       page: 0,
     });
    const [systemDetail, setSystemDetail] = useState<ISystemLogDetail[]>([]);
    const [rowCount, setRowCount] = useState(0);
    const [sortModel, setSortModel] = useState<GridSortModel>([
    { field: "systemTimestamp", sort: "desc" },
    ]);
    ...
    }, [open, serverID, paginationModel]);

    const getSystemLogs = async () => {
        const sortBy = sortModel[0]?.field || "systemTimestamp";
        const sortOrder = sortModel[0]?.sort === "desc" ? -1 : 1;
        try {
            const payload: IIdracServerSystemPayload = {
                pageNumber: paginationModel.page + 1,
                pageSize: paginationModel.pageSize,
                sortBy:sortBy,
                sortOrder: sortOrder,
                idracServerId: serverID,
            };

            const response: any = await fetchIdracSytemData(payload);
            if (response.isSuccess) {
                setSystemDetail(response?.data?.systemLogsDetails || []);
    ...
    const columns: GridColDef[] = [
        ...
        {
            field: "description",
            headerName: t("IDRAC_System_Logs.Grid.Description"),
            flex: 2,
            minWidth: 300,
            sortable: false,
        },        
        {
            ...
            headerName: t("IDRAC_System_Logs.Grid.System_Time"),
            flex: 1.5,
            renderCell: (params) => {            
        ...
        {
            field: "severity",
            headerName: t("IDRAC_System_Logs.Grid.Severity"),
            flex: 1,
            minWidth: 150,
            sortable: false,
            renderCell: (params) => {
...
export default SystemLogDialog