...
import { UploadBulkOwner } from "../../services/anprService";

type BulkUploadOwnerProps = {
  onClose: () => void;
  refreshData: () => void;
};

const BulkUploadOwner: React.FC<BulkUploadOwnerProps> = ({
    onClose,
    refreshData,
}) => {
   const [ownerFile, setOwnerFile] = useState<File | null>(null);
     ...
     const handleSaveFiles = async () => {
       if (!ownerFile) {
         showToast("Please upload owner files.", "error");
         return;
       }
   
       // Validate file extensions
       const ownerFileName = ownerFile.name.toLowerCase();
   
       const validExtensions = [".xls", ".xlsx"];
   
       const isValid = validExtensions.some((ext) => ownerFileName.endsWith(ext));
       ...
       try {
         const data = await UploadBulkOwner(formData);
         if (
           typeof data === "object" &&
           data !== null &&
           "isSuccess" in data &&
         ...
         <Grid className="add-license-pop-wrapper">
           <Grid item xs={10} md={10} lg={10}>
             <Typography variant="h6" fontWeight={600}>
              {t("ANPR_Screen.Owner.Upload_your_owner_file")}
             </Typography>
             ...
             <Box className="upload-your-file">
               {ownerFile ? (
                 <Box className="upload-your-file-wrapper">
                   ...
                   <IconButton onClick={handleOwnerFileEdit}>
               ...
               <input
                 ...
                 hidden
                 onChange={handleBulkOwnerUpload}
