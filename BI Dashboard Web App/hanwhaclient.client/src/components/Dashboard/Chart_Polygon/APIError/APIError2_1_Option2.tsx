...
const APIError2_1_Option2: React.FC<APIErrorCountProps> = ({
  customizedWidth,
  customizedHeight,
  finalapiErrordata
}) => {
  ...
  const hasValidData = finalapiErrordata &&
    finalapiErrordata.length > 0 &&
    finalapiErrordata.some(item => item.totalCount > 0);

  useEffect(() => {
    if (!hasValidData) return;

    if (!finalapiErrordata || finalapiErrordata.length === 0) return;

    finalapiErrordata = finalapiErrordata.filter(x => x.totalCount > 0);
    if (finalapiErrordata.length === 0) return;
    ...
    const maxCount = d3.max(finalapiErrordata, (d) => d.totalCount) || 1;
    ...
    const color = d3
      .scaleSequential()
      .domain([0, maxCount])
      .interpolator(d3.interpolateRgb("#ffffff", "#B2D5FF"));
  ...
  }, [finalapiErrordata, customizedWidth, customizedHeight, hasValidData]);
