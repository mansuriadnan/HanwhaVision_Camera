...
const Gender2_1_Option2: React.FC<GenderProps> = ({
  customizedWidth,
  customizedHeight,
  genderDataWithTime,
  startDate,
  ...
  type GenderKey = "maleCount" | "femaleCount" | "undefinedCount";
  const categories: { key: GenderKey; label: string; color: string }[] = [
    { key: "maleCount", label: "Male", color: "#D9C40A" },
    { key: "femaleCount", label: "Female", color: "#328BFF" },
    { key: "undefinedCount", label: "Undefined", color: "#717171" },
  ];
  ...
  useEffect(() => {
    if (!genderDataWithTime || !categories) return;
    const updatedParsedData: IParsedDataWithCategory = {};
    categories.forEach((cat) => {
      updatedParsedData[cat.key] = genderDataWithTime.map((d) => ({
  ...
  }, [genderDataWithTime, selectedInterval]);
      ...
      {parsedDataWithCategory && (
        <ChartTicksSelector
          ...
          onChartDataChange={(result) => {
            if (!result || !result.maleCount || !result.femaleCount || !result.undefinedCount) {
                setFinalData({ maleCount: [], femaleCount: [], undefinedCount:[] });
                return;
              }
              ...
              const adjustedData = {
                maleCount: result?.maleCount?.map((item: any) => ({
                  ...item,
                  value: item.value + (item.value * percentage) / 100,
                })),

                femaleCount: result?.femaleCount?.map((item: any) => ({
                  ...item,
                  value: item.value + (item.value * percentage) / 100,
                })),

                undefinedCount: result?.undefinedCount?.map((item: any) => ({
                  ...item,
                  value: item.value + (item.value * percentage) / 100,
                })),
              };
