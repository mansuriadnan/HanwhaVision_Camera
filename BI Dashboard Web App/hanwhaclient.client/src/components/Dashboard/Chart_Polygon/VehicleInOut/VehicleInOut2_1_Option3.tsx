...
const VehicleInOut2_1_Option3: React.FC<PVInOutProps> = ({
  customizedWidth,
  customizedHeight,
  selectedStartDate,
  selectedEndDate,
  ...
  useExportHandler({
    ...
    startDate: convertDateToISOLikeString(selectedStartDate), // Convert to string
    endDate: convertDateToISOLikeString(selectedEndDate), // Convert to string
    floor,
    zones,
    ...
    categories.forEach((cat, catIndex) => {
      ...
      if (cat.key === "inCount") {
        ...
        gradient
          .append("stop")
          .attr("offset", "0%")
          .attr("stop-color", "#FFE0B3"); // light top
        gradient
          .append("stop")
          .attr("offset", "100%")
          .attr("stop-color", "#FF9500"); // strong bottom
      } else if (cat.key === "outCount") {
        // Green gradient (Out)
        gradient
          .append("stop")
          .attr("offset", "0%")
          .attr("stop-color", "#C9F5B3"); // light top
        gradient
          .append("stop")
          .attr("offset", "100%")
          .attr("stop-color", "#3CBD00"); // strong bottom
      }
      ...
      MultiBarchartGroup.append("g")
        ...
        // .attr("fill", cat.color)
        .attr("fill", `url(#vgradient-${cat.key})`)
        .on("mouseover", (_, d) => {
          d3.select("#tooltip")
            .style("opacity", 1)
...
export { VehicleInOut2_1_Option3 };
