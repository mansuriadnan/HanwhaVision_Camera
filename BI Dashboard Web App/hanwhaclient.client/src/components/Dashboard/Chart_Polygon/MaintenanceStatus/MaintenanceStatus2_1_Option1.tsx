...
const MaintenanceStatus2_1_Option1: React.FC<MaintenanceStatusProps> = ({
  customizedWidth,
  customizedHeight,
  statusDataWithTime,
  startDate,
  ...
  const categories: { key: StatusKey; label: string; color: string }[] = [
    { key: "notStartedCount", label: "Not Started", color: "#F7FF0B" },
    { key: "inProgressCount", label: "In Progress", color: "#FF9C08" },
    { key: "reworkCount", label: "Rework", color: "#4A86FF" },
    { key: "doneCount", label: "Done", color: "#33FFEE" },
  ];
  ...
  useEffect(() => {
    if (!statusDataWithTime || !categories) return;
    // console.log("statusDataWithTime->", statusDataWithTime);
    const updatedParsedData: IParsedDataWithCategory = {};
    categories.forEach((cat) => {
      updatedParsedData[cat.key] = statusDataWithTime.map((d) => ({
  ...
  }, [statusDataWithTime, selectedInterval]);
...
export { MaintenanceStatus2_1_Option1 };
