...
const ForkliftSpeedDetection2_1_Option1: React.FC<
  IForkliftSpeedDetectionProps
> = ({
  customizedWidth,
  customizedHeight,
  ...
  useEffect(() => {
    if (!speedDetectionData) return;

    const updatedParsedData: IParsedDataWithCategory = {};

    updatedParsedData["FSDVData"] = speedDetectionData.map((d) => ({
      date: new Date(formatDateToConfiguredTimezone(d.dateTime)),
      value: d.queueCount,
    }));

    setParsedDataWithCategory(updatedParsedData);
  }, [speedDetectionData, selectedInterval]);

  useEffect(() => {
    if (!speedDetectionData || !finalData) return;
    ...
    chartGroup
      ...
      .attr("href", "/images/dashboard/pink_node_icon.svg")
      .style("cursor", "pointer")
      .style("pointer-events", "all")
      .on("mouseover", (_, d) => {
        d3.select("#tooltip")
          .style("opacity", 1)
          .html(`<strong>Forklift Speed</strong> : ${formatNumber(d.value)}`);
      })
      .on("mousemove", (event) => {
        d3.select("#tooltip")
      ...
      {parsedDataWithCategory && (
        <ChartTicksSelector
          ...
          onChartDataChange={(result) => {
            setFinalData(result?.["FSDVData"] as ParsedDataFormat[]);
          }}
        />
      )}
      <svg ref={svgRef}></svg>
...
export { ForkliftSpeedDetection2_1_Option1 };
