      ...
      return (
        <div style={{
         ...
         <table
          ...
          >
            ...
            <tbody>
              ...
              {adjustedGrouped.map((row) => {
                ...
                return (
                  ...
                  >
                     ...
                     <td style={tdStyle}>
                      {formatNumber(row.inCount)}
                    </td>
                    <td style={tdStyle}>
                      {formatNumber(row.outCount)}
                    </td>
                  </tr>
                );
              })}
              ...
              {(() => {
                ...
                const totalIn = adjustedGrouped.reduce(
                  (s, r) => s + (r.inCount || 0),
                  0
                );
                const totalOut = adjustedGrouped.reduce(
                  (s, r) => s + (r.outCount || 0),
                  0
                );
          ...
          {/* Summary Section */}
          {adjustedGrouped.length > 0 && (() => {
            const highest = adjustedGrouped.reduce((max, r) =>
              r.inCount > max.inCount ? r : max
            , adjustedGrouped[0]);

            const lowest = adjustedGrouped.reduce((min, r) =>
              r.inCount < min.inCount ? r : min
            , adjustedGrouped[0]);
              ...
              <div style={{ marginTop: 8 }}>
                ...
                <ul style={{ paddingLeft: 18 }}>
                  <li  style={{ lineHeight: "18px"}}>
                    Day {highestLabel} has highest people in count ({formatNumber(highest.inCount)})
                  </li>
                  <li  style={{ lineHeight: "18px"}}>
                    Day {lowestLabel} has lowest people in count ({formatNumber(lowest.inCount)})
                  </li>
                </ul>
              </div>
            );
