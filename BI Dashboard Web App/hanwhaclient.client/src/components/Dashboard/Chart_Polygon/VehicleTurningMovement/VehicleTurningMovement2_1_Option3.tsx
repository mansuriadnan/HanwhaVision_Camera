...
const VehicleTurningMovement2_1_Option3: React.FC<IVTurningMovmentProps> = ({
  customizedHeight,
  customizedWidth,
  vTurningMovementData,
  startDate,
  endDate,
  floor,
  zones,
  ...
  type GenderKey = "left" | "right" | "straight";
  const categories: { key: GenderKey; label: string; color: string }[] = [
    { key: "left", label: "Left", color: "#FF5683" },
    { key: "right", label: "Right", color: "#FFD900" },
    { key: "straight", label: "Straight", color: "#20A2FF" },
  ];
  ...
  useEffect(() => {
    if (!vTurningMovementData || !categories) return;
    const updatedParsedData: IParsedDataWithCategory = {};
    categories.forEach((cat) => {
      updatedParsedData[cat.key] = vTurningMovementData.map((d) => ({
        date: new Date(
  ...
  }, [vTurningMovementData, selectedInterval]);
...
export { VehicleTurningMovement2_1_Option3 };
