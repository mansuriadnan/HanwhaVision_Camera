...
import { AveragePeopleCountProps, IParsedDataWithCategory, ParsedDataFormat } from "../../../../interfaces/IChart";
...
const AveragePeopleCount1_1: React.FC<AveragePeopleCountProps> = ({
  ...
  const [totalInPeople, setTotalInPeople] = useState<number>(0);
  ...
  useEffect(() => {
    if (!averagePeopleCountChartData) return;

    // Total
    const total = averagePeopleCountChartData.reduce(
      (sum, d) => sum + (d.inCount ?? 0),
      0
    );
    setTotalInPeople(total);
      ...
      <Box className="widget-main-wrapper">
        ...
        <Box className="widget-main-footer">
          <Box className="widget-main-footer-value">
            ...
            <span>{formatNumber(totalInPeople ?? 0)}</span>
          </Box>
          {!openZoomDialog ? (
            <Box
