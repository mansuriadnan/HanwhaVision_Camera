...
const Age2_1_Option2: React.FC<AgeProps> = ({
  customizedWidth,
  customizedHeight,
  ageDataWithTime,
  startDate,
  ...
  type GenderKey = "youngCount" | "adultCount" | "seniorCount" | "unknownCount";
  const categories: { key: GenderKey; label: string; color: string }[] = [
    { key: "youngCount", label: "Young", color: "#A1A2FF" },
    { key: "adultCount", label: "Adult", color: "#8EE46D" },
    { key: "seniorCount", label: "Senior", color: "#C2C2C2" },
    { key: "unknownCount", label: "Unknown", color: "#FFBB8D" },
  ];
  ...
  useEffect(() => {
    if (!ageDataWithTime || !categories) return;
    const updatedParsedData: IParsedDataWithCategory = {};
    categories.forEach((cat) => {
      updatedParsedData[cat.key] = ageDataWithTime.map((d) => ({
  ...
  }, [ageDataWithTime, selectedInterval]);
