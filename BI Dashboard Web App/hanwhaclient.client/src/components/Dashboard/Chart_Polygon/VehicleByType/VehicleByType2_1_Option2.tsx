...
const VehicleByType2_1_Option2: React.FC<VehicleByTypeProps> = ({
  ...
  const categories: { key: VehicleCountKey; label: string; color: string }[] = [
    {
      key: type === "In" ? "truckInCount" : "truckOutCount",
      label: "Truck",
      color: "#FF4D8F",
    },
    {
      key: type === "In" ? "motorCycleInCount" : "motorCycleOutCount",
      label: "MotorCycle",
      color: "#9E4AFF",
    },
    {
      key: type === "In" ? "busInCount" : "busOutCount",
      label: "Bus",
      color: "#ACE322",
    },
    {
      key: type === "In" ? "bicycleInCount" : "bicycleOutCount",
      label: "Bicycle",
      color: "#39EFFF",
    },
    {
      key: type === "In" ? "carInCount" : "carOutCount",
      label: "Car",
      color: "#FFBD06",
    },
  ];
  ...
  }, [vehicleDataWithTime, selectedInterval, type]);
      ...
      {parsedDataWithCategory && (
        <ChartTicksSelector
          ...
          onChartDataChange={(result) => {
            ...
            if (
              !result ||
              !result.bicycleInCount ||
              !result.busInCount ||
              !result.carInCount ||
              !result.motorCycleInCount ||
              !result.truckInCount
            ) {
              setFinalData({
                bicycleInCount: [],
            ...
            const adjustedData = {
              bicycleInCount: result?.bicycleInCount?.map((item: any) => ({
                ...item,
                value: item.value + (item.value * percentage) / 100,
              })),

              busInCount: result?.busInCount?.map((item: any) => ({
                ...item,
                value: item.value + (item.value * percentage) / 100,
              })),

              carInCount: result?.carInCount?.map((item: any) => ({
                ...item,
                value: item.value + (item.value * percentage) / 100,
              })),

              motorCycleInCount: result?.motorCycleInCount?.map(
                (item: any) => ({
                  ...item,
                  value: item.value + (item.value * percentage) / 100,
                }),
              ...
              truckInCount: result?.truckInCount?.map((item: any) => ({
                ...item,
                value: item.value + (item.value * percentage) / 100,
              })),
            };
