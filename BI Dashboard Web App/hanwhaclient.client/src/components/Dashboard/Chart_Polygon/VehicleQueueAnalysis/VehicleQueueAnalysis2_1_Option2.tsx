...
const VehicleQueueAnalysis2_1_Option2: React.FC<IVQAProps> = ({
  customizedWidth,
  customizedHeight,
  VQAData,
  startDate,
  ...
  const drawLineChart = () => {
    ...
    chartGroup
      ...
      .on("mouseover", (_, d) => {
        d3.select("#tooltip")
          .style("opacity", 1)
          .html(`<strong>Vehicle Queue</strong> : ${formatNumber(d.value)}`);
      })
      .on("mousemove", (event) => {
        d3.select("#tooltip")
      ...
      {parsedDataWithCategory && (
        <ChartTicksSelector
          ...
          onChartDataChange={(result) => {
            setFinalData(result?.["VQAData"] as ParsedDataFormat[]);
          }}
        />
      )}
      <svg ref={svgRef}></svg>
...
export { VehicleQueueAnalysis2_1_Option2 };
