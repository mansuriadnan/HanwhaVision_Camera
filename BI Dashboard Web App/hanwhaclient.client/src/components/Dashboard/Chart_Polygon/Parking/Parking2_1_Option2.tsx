...
const Parking2_1_Option2: React.FC<ParkingProps> = ({
  parkingTreeChartData,
  customizedWidth,
  customizedHeight,
  startDate,
  endDate,
  ...
  useEffect(() => {
    if (!parkingTreeChartData || parkingTreeChartData.length === 0) return;
    ...
    //     children: parkingTreeChartData,
    //   })
    //   .sum((d) => d.totalParkingOccupancy);

    const root = d3
      .hierarchy<IParkingTreeChartData>(
        { children: parkingTreeChartData } as any,
        (d) => (d as any).children,
      )
      .sum((d) => d.totalParkingOccupancy);
    ...
    const maxCount =
      d3.max(parkingTreeChartData, (d) => d.totalParkingOccupancy) || 1;
    ...
    const color = d3
      .scaleSequential()
      .domain([0, maxCount])
      .interpolator(d3.interpolateRgb("#ffffff", "#80CE78"));
  ...
  }, [parkingTreeChartData, customizedWidth, customizedHeight]);
    ...
    >
      {parkingTreeChartData?.length === 0 ? (
        ...
        <svg
          ...
          height={customizedHeight}
          // height={Math.max(
          //   (parkingTreeChartData || []).length * 40,
          //   customizedHeight
          // )}
        />
      )}
