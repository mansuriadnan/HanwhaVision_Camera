...
const Parking2_1_Option3: React.FC<ParkingProps> = ({
  parkingChartData,
  customizedWidth,
  customizedHeight,
  startDate,
      ...
      {parsedDataWithCategory && (
        <ChartTicksSelector
          ...
          onChartDataChange={(result) => {
            ...
            const updatedOccupied = result.occupied?.map((item: any) => ({
              ...item,
              value: item.value < 0 ? 0 : item.value,
            }));

            const updatedAvailable = updatedOccupied?.map((item: any) => ({
              date: item.date,
              value: Math.max(0, capacity - item.value),
            }));
