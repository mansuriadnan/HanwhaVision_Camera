...
import vehicleUturnIcon from "/images/dashboard/Vehicle_U_Turn_detection_Node.svg";
import { useTimeFormatContext } from "../../../../context/TimeFormatContext";
import { getTimeTickFormatter } from "../../../../utils/formatTimeTick";

const VehicleUTurn2_1_Option1: React.FC<IVehicleUTurnProps> = ({
  customizedHeight,
  customizedWidth,
  vUTurnData,
  startDate,
  ...
  useEffect(() => {
    ...
    gradient
      .append("stop")
      .attr("offset", "0%")
      .attr("stop-color", "#6C5BFF00");

    gradient
      .append("stop")
      .attr("offset", "100%")
      .attr("stop-color", "#6C5BFF");
    ...
    (async () => {
      chartGroup
        ...
        .attr("href", vehicleUturnIcon)
        .style("cursor", "pointer")
        .style("pointer-events", "all")
        .on("mouseover", (_, d) => {
          d3.select("#tooltip")
            .style("opacity", 1)
            .html(`<strong>Vehicle U Turn</strong> : ${formatNumber(d.value)}`);
        })
        .on("mousemove", (event) => {
          d3.select("#tooltip")
      ...
      {parsedDataWithCategory && (
        <ChartTicksSelector
          ...
          onChartDataChange={(result) => {
            setFinalData(result?.["VUTData"] as ParsedDataFormat[]);
          }}
        />
      )}
      <svg ref={svgRef}></svg>
...
export { VehicleUTurn2_1_Option1 };
