...
import vehicleInWrongIcon from "/images/dashboard/Vehicle_in_Wrong_Direction_Node.svg";
import { useTimeFormatContext } from "../../../../context/TimeFormatContext";
import { getTimeTickFormatter } from "../../../../utils/formatTimeTick";

const VehicleInWrongDirection2_1_Option1: React.FC<
  IVehicleInWrongDirectionProps
> = ({
  customizedHeight,
  customizedWidth,
  ...
  setExportHandler,
    ...
    (async () => {
      chartGroup
        ...
        .attr("href", vehicleInWrongIcon)
        .style("cursor", "pointer")
        .style("pointer-events", "all")
        .on("mouseover", (_, d) => {
          d3.select("#tooltip")
            .style("opacity", 1)
            .html(`<strong>Wrong Direction</strong> : ${formatNumber(d.value)}`);
        })
        .on("mousemove", (event) => {
          d3.select("#tooltip")
        ...
        {parsedDataWithCategory && (
          <ChartTicksSelector
            ...
            onChartDataChange={(result) => {
              setFinalData(result?.["VWDData"] as ParsedDataFormat[]);
            }}
          />
        )}
        <svg ref={svgRef}></svg>
...
export { VehicleInWrongDirection2_1_Option1 };
