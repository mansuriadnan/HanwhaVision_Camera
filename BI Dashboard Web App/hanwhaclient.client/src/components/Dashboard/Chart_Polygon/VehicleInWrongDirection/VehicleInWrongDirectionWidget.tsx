...
import { fetchVehicleInWrongDirectionDataService } from "../../../../services/dashboardService";
...
const VehicleInWrongDirectionWidget: React.FC<CommonWidgetProps> = ({
  item,
  floor,
  zones,
  selectedStartDate,
  ...
  const [vWrongDirectionData, setVWrongDirectionData] = useState<IVQAData[]>(
    []
  );
  ...
  useEffect(() => {
    fetchVehicleInWrongDirectionData();
  ...
  useEffect(() => {
    if (IsToday) {
      ...
      multipleGetLiveData("VehicleWrongDirection", floor as string[], zones as string[]);
    }
  }, [floor, zones, IsToday]);

  useExportHandler({
    apiEndpoint: `${apiUrls.WrongWayAnalysis}/csv`,
  ...
  const fetchVehicleInWrongDirectionData = async () => {
    ...
    try {
      ...
      const response: any = await fetchVehicleInWrongDirectionDataService(
        data as IWidgetPayload
      );

      if (response?.data.length > 0) {
        setVWrongDirectionData(response?.data as IVQAData[]);
      } else {
        setVWrongDirectionData([]);
      }
    } catch (error) {
      console.error("Error fetching Vehicle wrong direction data:", error);
      throw error;
    } finally {
      setLoadingCount((c) => c - 1);
    }
    ...
    return (
      ...
      >
        <VehicleInWrongDirection1_1
          ...
          vWrongDirectionData={vWrongDirectionData}
...
export { VehicleInWrongDirectionWidget };
