...
const ShoppingcartHeatmapWidget: React.FC<IWidgetRequestProps> = ({
  item,
  floor,
  zones,
  selectedStartDate,
  ...
  const fetchHeatmapData = async (deviceValue: string) => {
    ...
    try {
      let requestHeatmapData = {
        deviceId: parts[0],
        channelNo: parts[1],
        heatmapType: "ShoppingCartHeatMap",
  ...
  const fetchDevicesByFloorZone = async () => {
    ...
    try {
      ...
      console.error("Error fetching ShoppingHeatmap data:", error);
      throw error;
    } finally {
      setLoadingCount((c) => c - 1);
    }
...
export { ShoppingcartHeatmapWidget };
