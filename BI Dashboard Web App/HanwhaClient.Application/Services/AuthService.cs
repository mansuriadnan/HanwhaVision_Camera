...
namespace HanwhaClient.Application.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<TokenResponseModel> LoginAsync(string username, string password)
        {
            ...
            var userData = await _usersRepository.GetUserByUsernameAsync(username,cancellationToken); ////get user by username
                ...
                var refreshTokenClaims = new List<Claim>
     {
        ...
        public async Task<FloorZoneDataAccessPermissionForLinkedServer> GetFloorZoneDataAccessPermissionAsync(string username, string password, CancellationToken cancellationToken)
        {
            var userData = await _usersRepository.GetUserByUsernameAsync(username, cancellationToken); ////get user by username
            var response = new FloorZoneDataAccessPermissionForLinkedServer();
            ...
            var roleData = await _roleRepository.GetAllAsync(cancellationToken: cancellationToken);

            var roleScreenMappingsData = await _roleScreenMappingRepository.GetAllAsync(cancellationToken: cancellationToken);

            var roles = roleData.Where(x => userData.RoleIds.Contains(x.Id)).Select(y => y.Id).ToList();

            IEnumerable<FloorDataAccessPermission> floorZonePermissions = roleScreenMappingsData
                                                    .Where(x => roles.Any(y => y == x.RoleId))
                                                    .SelectMany(x => x.DataAccessPermissions).AsEnumerable();


            response.DataAccessPermissions =  floorZonePermissions;
